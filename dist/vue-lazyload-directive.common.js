...
Lazyload.prototype.listener = function listener () {
  document.addEventListener("scroll", this.handler.bind(this), true);
};
Lazyload.prototype.handler = function handler () {
  !this.throttleFn && (this.throttleFn = throttle(this.scroll, 300));
  this.throttleFn();
};
Lazyload.prototype.add = function add (elm) {
  this.elms[elm.key] = elm;
};
Lazyload.prototype.remove = function remove (key) {
  delete this.elms[key];
};
Lazyload.prototype.scroll = function scroll () {
    var this$1 = this;

  var keys = Object.keys(this.elms);
  for (var i = 0, list = keys; i < list.length; i += 1) {
    var key = list[i];

      if (this$1.elms[key].dataset.success) {
      continue;
    }
    if (this$1.isElementInViewport(this$1.elms[key])) {
      this$1.elms[key].execute();
    } else {
      // this.refs[key].restore();
    }
  }
};
Lazyload.prototype.isElementInViewport = function isElementInViewport (el) {
...
var wrapper = function (el, url) {
  el.execute = function () {
    if (el.dataset.success) { return; }
    var image = new Image();
    image.src = url;
    image.onload = image.onerror = function (ref) {
      var type = ref.type;

      el.style.opacity = 0.2;
      setTimeout(function () {
        var address = type === "error" ? lazy.failed : url;
